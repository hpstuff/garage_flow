...
describe("configFromInput (GF-20)", () => {
  it("maps the flat mon..sun form to the ISO-weekday-keyed ScheduleConfig", () => {
    const config = configFromInput({ enabled: true, weekly: DEFAULT_WEEKLY_INPUT, exceptions: [] });
  ...
  it("carries the enabled flag through as-is", () => {
...
describe("parseStoredConfig (GF-20)", () => {
  it("falls back to the default schedule for null/empty stored values", () => {
    const config = parseStoredConfig(null, true);
    expect(config.enabled).toBe(true);
  ...
  it("reflects whatever enabled it's given, independent of the stored JSON", () => {
    expect(parseStoredConfig(null, false).enabled).toBe(false);
    expect(parseStoredConfig({ weekly: {} }, false).enabled).toBe(false);
  });
  ...
  it("round-trips a config written by configFromInput (the persisted shape)", () => {
    ...
    const read = parseStoredConfig(written, written.enabled);
    expect(read).toEqual(written);
  });

  it("fills in missing weekdays from the default rather than crashing", () => {
    const read = parseStoredConfig({ weekly: { "1": { start: "08:00", end: "12:00" } } }, true);
...
describe("getDayHours + validateAppointmentWithinSchedule (GF-20)", () => {
  ...
  it("allows any time at all when the Location has schedule enforcement disabled", () => {
    const config = configFromInput({
      enabled: false,
      weekly: DEFAULT_WEEKLY_INPUT,
      exceptions: [],
    });
    // Outside the weekly hours entirely — would be blocked if enforcement were on.
    const error = validateAppointmentWithinSchedule(
      { startsAt: new Date(2026, 7, 17, 3, 0), endsAt: new Date(2026, 7, 17, 4, 0) },
      config,
    );
    expect(error).toBeNull();
  });
