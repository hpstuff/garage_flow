...
function toFormInput(config: ScheduleConfig | null): ScheduleConfigInput {
  if (!config) {
    return { enabled: true, weekly: { ...DEFAULT_WEEKLY_INPUT }, exceptions: [] };
  }
  ...
  return { enabled: config.enabled, weekly, exceptions };
}
...
export function ScheduleSettingsForm({ config }: { config: ScheduleConfig | null }) {
  ...
  async function onSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    // This form only renders while the feature is enabled (see ScheduleEnabledToggle).
    const result = await setScheduleConfigAction({ enabled: true, weekly, exceptions });
